...
export function getAiThreadMessages(ctx: QueryCtx, args: TGetAiThreadMessagesArgs) {
  return ResultAsync.fromPromise(
    storeAgent.listMessages(ctx, {
      threadId: args.threadId,
      paginationOpts: args.paginationOpts,
    }),
    (e) => Errors.getAiThreadMessagesFailed({ message: "Failed to get AI thread messages" })
  ).andThen((messages) => {
    return ResultAsync.fromPromise(
      ...
      (e) => Errors.getAiThreadMessagesFailed({ message: "Failed to sync streams" })
    ).andThen((streams) => {
